...
class SettingsRepositoryImpl
    ...
    constructor(
        ...
        override val displayMode: Flow<DisplayMode> =
            dataStore.displayMode.map { mode ->
                when (mode) {
        ...
        override val translationEngine: Flow<TranslationEngine> =
            dataStore.translationEngine.map { engine ->
                when (engine) {
                    "FAST", "HIGH_QUALITY" -> TranslationEngine.LOCAL
                    else ->
                        try {
                            TranslationEngine.valueOf(engine)
                        } catch (_: Exception) {
        ...
        override suspend fun setTranslationEngine(engine: TranslationEngine) = dataStore.setTranslationEngine(engine.name)

        override val cloudTranslationProvider: Flow<CloudTranslationProvider> =
            dataStore.cloudTranslationProvider.map { provider ->
                try {
                    CloudTranslationProvider.valueOf(provider)
                } catch (_: Exception) {
        ...
        override suspend fun setCloudTranslationApiKey(key: String) = dataStore.setCloudTranslationApiKey(key)
        ...
        override val isLlmConfigReady: Flow<Boolean> =
