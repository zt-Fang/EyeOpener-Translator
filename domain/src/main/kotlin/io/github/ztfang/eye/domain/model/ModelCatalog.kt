...
object ModelCatalog {
     ...
     */
    val VOSK_MODELS: Map<String, ModelFileSpec> =
        VoskLanguage.getAll().associate { lang ->
            lang.code to
                ModelFileSpec(
     ...
     */
    val SHERPA_ONNX_MODELS: Map<String, ModelFileSpec> =
        SherpaOnnxModel.getAll().associate { model ->
            model.modelId to
                ModelFileSpec(
    ...
    fun totalSizeBytes(name: String): Long =
        when {
    ...
    fun filesFor(name: String): List<ModelFileSpec> =
        when {
            name.startsWith("VOSK_ASR_") -> {
                val lang = name.substringAfter("VOSK_ASR_").lowercase()
    ...
    fun voskZipSpec(languageCode: String): ModelFileSpec? = VOSK_MODELS[languageCode.lowercase()]

    /** 获取指定模型 ID 的 Sherpa-ONNX tar.bz2 模型规格 */
    fun sherpaOnnxTarSpec(modelId: String): ModelFileSpec? = SHERPA_ONNX_MODELS[modelId]

    /** 多文件直链规格；null 表示走 tar.bz2 下载流程 */
    fun sherpaOnnxFileSpecs(modelId: String): List<ModelFileSpec>? = SherpaOnnxModel.fromModelId(modelId)?.files
}
